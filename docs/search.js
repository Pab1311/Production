...
$.widget("custom.catcomplete", $.ui.autocomplete, {
  ...
  _renderItem: function (ul, item) {
    ...
    if (item.category === catSearchTags) {
      if (item.d) {
        $("<a/>").attr("href", "#")
        .html(label + "<span class=\"searchTagHolderResult\"> (" + item.h
...
$(function () {
  $("#search").catcomplete({
    ...
    source: function (request, response) {
      ...
      camelCaseRegexp = ($.ui.autocomplete.escapeRegex(request.term)).split(
          /(?=[A-Z])/).join("([a-z0-9_$]*?)");
      var camelCaseMatcher = new RegExp("^" + camelCaseRegexp);
      secondaryMatcher = new RegExp($.ui.autocomplete.escapeRegex(request.term),
          "i");
